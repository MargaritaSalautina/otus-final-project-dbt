...
LEFT JOIN {{ ref('cv_last_order_status') }} AS ls on ls.ORDER_PK = so.ORDER_PK
LEFT JOIN {{ ref('sat_order_status') }} AS os on os.ORDER_STATUS_PK = ls.ORDER_STATUS_PK
LEFT JOIN {{ ref('link_order_currency') }} AS oc on oc.ORDER_PK = so.ORDER_PK
)
select
    ords.ORDER_PK,
    ords.CURRENCY_PK,
...
from ords
LEFT JOIN {{ ref('link_order_goods') }} AS og on og.ORDER_PK = ords.ORDER_PK
LEFT JOIN {{ ref('sat_good') }} AS sg on sg.GOOD_PK = og.GOOD_PK
LEFT JOIN {{ ref('cv_currency_rate_by_date') }} AS crr on crr.CURRENCY_PK = ords.CURRENCY_PK
