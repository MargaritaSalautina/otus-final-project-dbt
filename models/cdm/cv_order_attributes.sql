...
from ords
LEFT JOIN {{ ref('cv_last_order_status') }} AS ls on ls.ORDER_PK = ords.ORDER_PK
LEFT JOIN {{ ref('sat_order') }} AS so on ords.ORDER_PK = so.ORDER_PK
LEFT JOIN {{ ref('cv_order_clients_info') }} AS oc on oc.ORDER_PK = ords.ORDER_PK
LEFT JOIN {{ ref('cv_business_area') }} AS ba on ba.receiver_type_name = oc.sender_client_type_name
