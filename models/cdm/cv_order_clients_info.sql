...
LEFT JOIN {{ ref('cv_order_payer') }} AS cop on cop.ORDER_PK = so.ORDER_PK
LEFT JOIN {{ ref('sat_client_type') }} AS ctp on ctp.CLIENT_TYPE_PK = cop.CLIENT_TYPE_PK
LEFT JOIN {{ ref('cv_order_sender') }} AS cos on cos.ORDER_PK = so.ORDER_PK
LEFT JOIN {{ ref('sat_client_type') }} AS cts on cts.CLIENT_TYPE_PK = cos.CLIENT_TYPE_PK
LEFT JOIN {{ ref('cv_order_receiver') }} AS cor on cor.ORDER_PK = so.ORDER_PK
LEFT JOIN {{ ref('sat_client_type') }} AS ctr on ctr.CLIENT_TYPE_PK = cor.CLIENT_TYPE_PK